...
export class PostService {
    ...
    findAllPostsByUsuario(usuario: String): Observable<HttpResponse<IPost[]>> {
        return this.http
            .get(`${this.resourceUrl}/muro/${usuario}`, { observe: 'response' })
            .pipe(map((res: HttpResponse<IPost[]>) => this.convertDateArrayFromServer(res)));
    }

    findPostWallByUsuarioId(id: number): Observable<HttpResponse<IPost[]>> {
        return this.http
            .get(`${this.resourceUrl}/muro/postwall-files/${id}`, { observe: 'response' })
            .pipe(map((res: HttpResponse<IPost[]>) => this.convertDateArrayFromServer(res)));
    }

    findPostWallByUsuarioIdPageable(id: number, page: number, size: number): Observable<HttpResponse<IPost[]>> {
    ...
    findPostWallByUsuarioIdPageableVisitedProfile(id: number, page: number, size: number): Observable<HttpResponse<IPost[]>> {
        const httpParams: HttpParams = new HttpParams()
            .set('idUsuario', id.toString())
            .set('page', page.toString())
            .set('size', size.toString());
    ...
    protected convertDateFromClient(post: IPost): IPost {
        const copy: IPost = Object.assign({}, post, {
            fechaPublicacion:
                post.fechaPublicacion != null && post.fechaPublicacion.isValid() ? post.fechaPublicacion.format(DATE_TIME_FORMAT) : null
        });
        return copy;
    }
